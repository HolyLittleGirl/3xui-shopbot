...
При включении RKN домены загружаются из локального кэша:
- Файл: `/opt/rkn-blocker/rkn_domains.json`
- По умолчанию: тестовый список (3 домена)
- Полный список: ~43,000 доменов (загружается вручную)

Для загрузки полного списка:
```bash
python3 /opt/rkn-blocker/update-3xui-rkn-domains.py download
```

Домены разбиваются на правила по 1000 штук и добавляются в 3x-ui routing.
