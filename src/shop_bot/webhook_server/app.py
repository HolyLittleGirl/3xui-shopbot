...
def create_webhook_app(bot_controller_instance):
    ...
    def add_plan_route():
        try:
            price_stars = int(request.form.get('price_stars', 0) or 0)
    ...
    def update_plan_route(plan_id):
        ...
        price_stars = request.form.get('price_stars', '0')
        try:
            months_int = int(months)
            price_float = float(price)
            price_stars_int = int(price_stars) if price_stars else 0
        except (TypeError, ValueError):
        ...
        ok = update_plan(plan_id, plan_name, months_int, price_float, price_stars_int)
        if ok:
