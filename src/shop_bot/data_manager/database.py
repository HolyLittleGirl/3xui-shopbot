...
def run_migration():
    ...
    try:
        ...
        if table_exists:
            ...
            if 'price_stars' not in p_columns:
                cursor.execute("ALTER TABLE plans ADD COLUMN price_stars INTEGER DEFAULT 0")
                logging.info(" -> Столбец 'price_stars' успешно добавлен в 'plans'.")
            else:
                logging.info(" -> Столбец 'price_stars' уже существует в 'plans'.")
        else:
            logging.warning("Таблица 'plans' не найдена.")
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                (host_name, plan_name, months, price, price_stars)
            )
            conn.commit()
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                (plan_name, months, price, price_stars, plan_id)
            )
            conn.commit()
            if cursor.rowcount == 0:
                logging.warning(f"No plan updated for id {plan_id} (not found).")
                return False
            logging.info(f"Updated plan {plan_id}: name='{plan_name}', months={months}, price={price}, price_stars={price_stars}.")
            return True
    except sqlite3.Error as e:
...
def update_key_host_and_info(key_id: int, new_host_name: str, new_xui_uuid: str, new_expiry_ms: int, new_email: str | None = None):
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if new_email:
                cursor.execute(
                    "UPDATE vpn_keys SET host_name = ?, xui_client_uuid = ?, expiry_date = ?, key_email = ? WHERE key_id = ?",
                    (new_host_name, new_xui_uuid, expiry_date, new_email, key_id)
                )
            else:
                cursor.execute(
